...
For iOS, use the general production profile:

```powershell
eas build --platform ios --profile production
```

The iOS configuration uses bundle identifier `com.gabri.tuonta`, supports iPhone and iPad layouts, and includes the TuonTa! icon. EAS may ask for an Apple Developer account and iOS signing credentials. The resulting iOS build must be installed through the distribution method selected by EAS; the Android APK download cannot be installed on iOS.
...
The screens use `react-native-safe-area-context` through `SafeAreaProvider` so headers and forms clear Android status bars when edge-to-edge rendering is enabled. Home header controls are constrained for narrow phone widths, while the Tasks and Profile screens retain scrollable responsive layouts. After any layout change, a new native build is required to test the result on a physical device. Use the iOS EAS command above for iPhone/iPad testing and the Android APK profile for Android testing.
