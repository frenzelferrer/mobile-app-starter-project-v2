...
export function AddTaskScreen({ navigation }: Props) {
  ...
  const handleSubmit = (values: TaskFormData) => {
    addTask(values);
    Alert.alert("Task Created", "Your task was successfully added.", [
      { text: "View tasks", onPress: () => navigation.popToTop() },
    ]);
  };
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
      <Text style={[styles.title, { color: colors.text }]}>Add a task</Text>
      <Text style={[styles.subtitle, { color: colors.muted }]}>Turn an upcoming deadline into a clear next step.</Text>
      <TaskForm
